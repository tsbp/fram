...
#define SELECT()  P4OUT &= ~CS_BIT;//PORTB &= ~DF_SS_PB;  // Ножка SS
#define DESELECT() P4OUT |= CS_BIT;//PORTB |= DF_SS_PB;
...
static const df_Info_t DfInfo = { 8192, 1056, 11, (0xF<<2) };
...
#define ContArrayRead			0xe8	// Continuous Array Read (Note : Only A/B/D-parts supported)
...
uint8 df_DetectParams()
{
   ...
   for (uint8 i=0; i<sizeof(df_table); i++)
   {
